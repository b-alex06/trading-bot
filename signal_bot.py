...
def get_data(pair: str, interval: str = "4h") -> pd.DataFrame:
    """Slim: probeer Twelve Data eerst, dan Yahoo Finance als fallback"""
    if TWELVE_DATA_KEY != "JOUW_TWELVE_DATA_KEY_HIER":
        df = get_data_twelvedata(pair, interval)
        if len(df) >= 50:
            return df
    # Fallback naar Yahoo Finance
    return get_data_yfinance(pair, interval)
...
class TechnicalAnalysis:
    ...
    def full_analysis(self):
        ...
        try:
            dxy=yf.Ticker("DX-Y.NYB"); ddf=dxy.history(period="5d",interval="1h")
            if len(ddf)>5:
                dxy_ema=ddf['Close'].ewm(span=20).mean().iloc[-1]
                dxy_b=ddf['Close'].iloc[-1]>dxy_ema; dxy_s=not dxy_b
